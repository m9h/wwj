...
def run_one(a, b, snr, M, seed, P):
    ...
    w_est, g2_naive = spectra_raw(X, y)
    ...
    return skill(cbp_naive, cbp_true), skill(cbp_robust, cbp_true)
...
def main():
    ...
    for a, b, snr in scenarios:
        ...
        print(f"{'M':>9s} {'p/M':>6s} | {'skill_naive':>11s} {'skill_robust':>12s}")
        for M in MS:
            sn, sr = zip(*(run_one(a, b, snr, M, s, P) for s in SEEDS))
            print(f"{M:9d} {P/M:6.2f} | {np.mean(sn):+11.2f} {np.mean(sr):+12.2f}", flush=True)
        print()
